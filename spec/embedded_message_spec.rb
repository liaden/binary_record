...
describe "Embedded Messages" do
  context 'with default config' do
    ...
    describe "with polymorphic relationship" do
      let(:message) do
        EmbeddedMessagePolymorphicType.create(:my_message => Uint16Table.create(:field => 6))  
      end
      it "creates instance" do
      ...
      it 'binary string changes for different associated type' do
        text = message.to_binary_s
        message.my_message = Uint8Table.create(:field => 7)
        message.to_binary_s.should_not == text  
      end 
      ...
      it "parses from a binary string" do
        msg = EmbeddedMessagePolymorphicType.read(message.to_binary_s)
        msg.should be_valid
        msg.my_message.field.should == 6
      end
    end
  end
end
